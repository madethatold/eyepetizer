...
public class MyRefreshHeader extends LinearLayout implements RefreshHeader {
    ...
    public MyRefreshHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        View view = View.inflate(context, R.layout.my_refresh_header, this);
        tv = view.findViewById(R.id.tv_header);
    }
    ...
    public void onMoving(boolean isDragging, float percent, int offset, int height, int maxDragHeight) {
        ...
        if(percent>=0.25&&percent<0.375){
            tv.setText("——下拉");
        }
        if (percent>=0.375&&percent<0.5){
            tv.setText("——下拉关");
        }
        if (percent>=0.5&&percent<0.625){
            tv.setText("——下拉关闭");
        }
        if (percent>=0.625&&percent<0.75){
            tv.setText("——下拉关闭页");
        }
